        ...
        try {
          ...
          if (useGasless) {
            ...
            try {
              ...
              logger.info(
                `Standard swap failed (insufficient lamports), retrying gasless (${user.discordId})`
              );
              await interaction.editReply({
          ...
          logger.info(
            `Swap success${usedGasless && !useGasless ? ' (gasless fallback)' : ''}: ${signature} (${user.discordId})`
          );
        } catch (error) {
