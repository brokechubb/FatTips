...
const { Connection, PublicKey } = require(
  path.join(__dirname, '../packages/solana/node_modules/@solana/web3.js')
);
const { getAssociatedTokenAddress, getAccount } = require(
  path.join(__dirname, '../packages/solana/node_modules/@solana/spl-token')
);
...
async function main() {
  ...
  if (!dbUrl || !rpcUrl) {
    console.log('Usage: node check-pool-balances.js <DATABASE_URL> <RPC_URL>');
    console.log('Example: node check-pool-balances.js postgresql://... https://...');
    process.exit(1);
  }
  ...
  try {
    ...
    const poolWallets = await prisma.airdropPoolWallet.findMany({
